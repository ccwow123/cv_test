...
def initializeTrackbars_Hough(intialTracbarVals=0):
    cv2.namedWindow("Trackbars")
    cv2.resizeWindow("Trackbars", 500, 300)
    cv2.createTrackbar("p1", "Trackbars", 100, 255, nothing)#Canny边缘检测器的高阈值 默认值100
    cv2.createTrackbar("p2", "Trackbars", 100, 255, nothing)#默认值100
    cv2.createTrackbar("minRadius", "Trackbars", 0, 255, nothing)
    cv2.createTrackbar("maxRadius", "Trackbars", 0, 20, nothing)
    cv2.createTrackbar("minDist", "Trackbars", 100, 200, nothing)


def valTrackbars_Hough():
    Threshold1 = cv2.getTrackbarPos("p1", "Trackbars")
    Threshold2 = cv2.getTrackbarPos("p2", "Trackbars")
    Threshold3 = cv2.getTrackbarPos("minRadius", "Trackbars")
    Threshold4 = cv2.getTrackbarPos("maxRadius", "Trackbars")
    Threshold5 = cv2.getTrackbarPos("minDist", "Trackbars")


    src = Threshold1, Threshold2, Threshold3, Threshold4, Threshold5
    return src
def mkdir(path):
    # 去除首位空格
    path = path.strip()

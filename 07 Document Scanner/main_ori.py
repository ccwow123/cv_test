...
pathImage = r'C:\Users\18493\Desktop\cap' #关闭摄像头时图片文件夹
ext='bmp'
...
utlis.initializeTrackbars()#参数滑动条
count = 0
...
while True:
    ...
    if webCamFeed:
        ...
        img_list = utlis.getFileList2(pathImage,ext)
    ...
    # ret,imgBlur=cv2.threshold(imgGray,127 , 255, cv2.THRESH_BINARY )
    thres = utlis.valTrackbars()  # 实例化阈值的滑动条
    imgThreshold = cv2.Canny(imgBlur, thres[0], thres[1])  # 边缘检测器
    kernel = np.ones((5, 5))#图像处理的卷积核
    imgDial = cv2.dilate(imgThreshold, kernel, iterations=1)  # 图像处理：膨胀

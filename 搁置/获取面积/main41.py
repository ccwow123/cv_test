...
def get_target_true_area(path,path2,reference_width= 1.3225,reference_height= 1.3225):
    ...
    result_img = cv2.drawContours(img.copy(), cnts, -1, (255, 0, 0), 1)
    # ==显示面积文字
    for i2 in range(len(target_area)):
        result_img = cv2.putText(result_img, target_area0 + 'mm2', (box_list[i2]), cv2.FONT_HERSHEY_COMPLEX, 0.8,
                                 (255, 0, 0), 2)
    # 保存图片
    save_path=r'C:\Users\18493\Desktop\tt2\11'
    print(save_path + '/' + "1.jpg")
    cv2.imwrite(save_path + '/' + "1.jpg", cv2.cvtColor(result_img, cv2.COLOR_RGB2BGR))
    print('缺陷面积为：%.5f mm2' % true_area)
    return true_area


if __name__ == '__main__':
    path = r'D:\Files\_datasets\voc_self\data_dataset_voc\JPEGImages\E_pinhole_16.jpg'  # 电容图片
    path2 = r'D:\Files\_datasets\voc_self\data_dataset_voc\SegmentationClass\E_pinhole_16.png'  # 缺陷mask图片   注意路径不能有中文
